...
// ── StickyTracker position cache ──────────────────────────────────────
// getItemTop() reads yogaNode.getComputedTop() which is expensive when
// called 30-60 times per scroll tick. Cache the results in a module-level
// Map keyed by item index. Invalidate when the mounted range [start, end)
// changes or when layout is recomputed (detected via a generation counter).
let _stickyTopCache = new Map<number, number>();
let _stickyRangeStart = -1;
let _stickyRangeEnd = -1;
let _stickyLayoutGen = 0;

function getCachedItemTop(
  index: number,
  getItemTop: (index: number) => number,
  layoutGen: number,
): number {
  if (layoutGen !== _stickyLayoutGen) {
    // Layout generation changed — full invalidation.
    _stickyTopCache = new Map<number, number>();
    _stickyLayoutGen = layoutGen;
  }
  const cached = _stickyTopCache.get(index);
  if (cached !== undefined) return cached;
  const top = getItemTop(index);
  if (top >= 0) {
    _stickyTopCache.set(index, top);
  }
  return top;
}
...
function StickyTracker({
  ...
  // Invalidate position cache when the mounted range changes (layout
  // recomputed by the parent's coarse-quantum render). Bump the generation
  // counter so getCachedItemTop() clears stale entries.
  if (start !== _stickyRangeStart || end !== _stickyRangeEnd) {
    _stickyRangeStart = start;
    _stickyRangeEnd = end;
    _stickyLayoutGen++;
  }
  const layoutGen = _stickyLayoutGen;
  ...
  for (let i = end - 1; i >= start; i--) {
    const top = getCachedItemTop(i, getItemTop, layoutGen);
    if (top >= 0) {
      if (top < target) break;
      firstVisibleTop = top;
    }
