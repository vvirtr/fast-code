...
// [fast-code] Settled color: electric blue instead of grey
const SETTLED_BLUE = toRGBColor({
  r: 30,
  g: 144,
  b: 255
});
export function AnimatedAsterisk({
  ...
  const [ref, time] = useAnimationFrame(done ? null : 40);
  useEffect(() => {
    if (done) return;
  ...
  if (done) {
    return <Box ref={ref}>
        <Text color={SETTLED_BLUE}>{char}</Text>
      </Box>;
  }
  ...
  // [fast-code] Blue pulse animation instead of rainbow sweep
  const progress = (elapsed % SWEEP_DURATION_MS) / SWEEP_DURATION_MS;
  const pulse = Math.sin(progress * Math.PI);
  const color = toRGBColor({
    r: Math.round(30 + pulse * 100),
    g: Math.round(144 + pulse * 80),
    b: Math.round(255),
  });
  return <Box ref={ref}>
