 ...
 *
 * When `lazyContentPath` is provided, the markdown body is not held in memory
 * at discovery time.  Instead, the file is re-read (and cached) on the first
 * invocation of `getPromptForCommand`.  This keeps the startup footprint small
 * when many skills are registered but only a few are actually invoked.
 */
export function createSkillCommand({
  ...
  effort: EffortValue | undefined
  shell: FrontmatterShell | undefined
  /** When set, the body is read lazily from this path on first invocation. */
  lazyContentPath?: string
}): Command {
  // Lazy content cache: when lazyContentPath is provided, the markdownContent
  // passed at construction is only used for contentLength; the actual body is
  // re-read from disk on first getPromptForCommand call.
  let cachedContent: string | undefined = lazyContentPath
    ? undefined
    : markdownContent
  ...
  async function resolveContent(): Promise<string> {
    ...
    // Lazy read from disk on first invocation
    const fs = getFsImplementation()
    const raw = await fs.readFile(lazyContentPath!, { encoding: 'utf-8' })
    const { content } = parseFrontmatter(raw, lazyContentPath!)
