...
|------|-------|-----|---------|
| \`~/.fast/settings.json\` | Global | N/A | Personal preferences for all projects |
...
Hooks run commands at specific points in Fast Code's lifecycle.
...
\`\`\`json
{
  "hooks": {
    "PreToolUse": [{
      ...
      "hooks": [{
        "type": "command",
        "command": "jq -r '.tool_input.command' >> ~/.fast/bash-log.txt"
      }]
    }]
  }
}
...
Modify Fast Code configuration by updating settings.json files.
...
1. **Check the settings file** - Read ~/.fast/settings.json or .claude/settings.json
