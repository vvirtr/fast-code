...
const FILE_FORMAT_EXAMPLE: KeybindingsSchemaType = {
  ...
  $docs: 'https://github.com/vvirtr/fast-code',
  bindings: [
    {
      context: 'Chat',
      bindings: {
...
const SECTION_INTRO = [
  '# Keybindings Skill',
  '',
  'Create or modify `~/.fast/keybindings.json` to customize keyboard shortcuts.',
  '',
  '## CRITICAL: Read Before Write',
  '',
  '**Always read `~/.fast/keybindings.json` first** (it may not exist yet). Merge changes with existing bindings — never replace the entire file.',
  '',
...
const SECTION_DOCTOR = [
  ...
  '',
  'The `/doctor` command includes a "Keybinding Configuration Issues" section that validates `~/.fast/keybindings.json`.',
  '',
  ...
  'Location: ~/.fast/keybindings.json',
...
export function registerKeybindingsSkill(): void {
  registerBundledSkill({
    ...
    description:
      'Use when the user wants to customize keyboard shortcuts, rebind keys, add chord bindings, or modify ~/.fast/keybindings.json. Examples: "rebind ctrl+s", "add a chord shortcut", "change the submit key", "customize keybindings".',
    allowedTools: ['Read'],
    userInvocable: false,
