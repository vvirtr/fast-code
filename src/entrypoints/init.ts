...
export const init = memoize(async (): Promise<void> => {
  ...
  try {
    ...
    // [fast-code] Stripped: 1P event logging, GrowthBook refresh, OAuth populate,
    // JetBrains detection, git repo detection — none essential for core functionality
    ...
    // [fast-code] Stripped: recordFirstStartTime (analytics only)
    ...
    // [fast-code] Stripped: preconnectAnthropicApi (marginal benefit, adds complexity)
...
export function initializeTelemetryAfterTrust(): void {
  // [fast-code] Telemetry stripped — no-op
  return
}

function _originalInitializeTelemetryAfterTrust(): void {
  if (isEligibleForRemoteManagedSettings()) {
