...
// [fast-code] Enable session-memory compaction (avoids expensive API compact calls)
...
async function main(): Promise<void> {
  ...
  // [fast-code] Clear the shell prompt line ("user@host:~$ fast") before UI renders.
  // Move cursor up to the line with the command, clear it, then return.
  // Only for interactive TTY, skip for -p/--print/--version/piped output.
  if (process.stdout.isTTY && !args.includes('-p') && !args.includes('--print')) {
  ...
  if (args.length === 1 && (args[0] === '--version' || args[0] === '-v' || args[0] === '-V')) {
    ...
    console.log(`${MACRO.VERSION} (Fast Code)`);
    return;
  }
  ...
  // Session management against the ~/.fast/sessions/ registry. Flag
  ...
  // Fast-path for -p/--print: detect headless mode before loading main.tsx.
  // main.tsx has 155+ static imports (React, Ink, Yoga, commander, etc.) that
  // are unnecessary for headless/print mode — print.ts is already React-free.
  //
  // TODO(perf): bypass main.tsx entirely for -p/--print by directly calling
  // runHeadless from cli/print.ts. This requires extracting the shared setup
  // logic from main.tsx's action handler (~1600 lines between the commander
  // action entry and the runHeadless call) into a reusable module. Key setup
  // steps that runHeadless depends on:
  //   1. init() (enableConfigs, applySafeConfigEnvironmentVariables, proxy, mTLS)
  //   2. setup() (session ID, cwd, worktree, sinks, git root detection)
  //   3. Tool loading (getTools, assembleToolPool, mergeAndFilterTools)
  //   4. MCP configuration (parseMcpConfigs, connectMcpBatch, claudeai configs)
  //   5. Agent definitions (getAgentDefinitionsWithOverrides)
  //   6. Permission mode resolution and headlessStore creation
  //   7. Model resolution (parseUserSpecifiedModel, ensureModelStringsInitialized)
  //   8. GrowthBook initialization (analytics gates)
  //   9. Policy limits loading
  //  10. Commands loading (getCommands)
  //
  // Until that extraction is done, -p/--print falls through to the full
  // main.tsx path below. The detection here is ready for the fast-path to be
  // wired up once a shared setupHeadless() function is available:
  //
  //   if (hasPrintFlag) {
  //     const { setupHeadless } = await import('../headlessSetup.js');
  //     const ctx = await setupHeadless(args);
  //     const { runHeadless } = await import('../cli/print.js');
  //     await runHeadless(ctx.prompt, ...ctx.params);
  //     return;
  //   }
  //
  const hasPrintFlag = args.includes('-p') || args.includes('--print');
  if (hasPrintFlag) {
    profileCheckpoint('cli_print_detected');
    // For now, fall through to main.tsx. When the shared setup module is
    // extracted, this block will directly import and call runHeadless,
    // avoiding the ~135ms of main.tsx module evaluation overhead (React,
    // Ink, Yoga, commander, and 150+ other static imports).
  }
