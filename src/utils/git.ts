...
import { memoize } from './lodashNative.js'
    ...
    while (current !== root) {
      try {
        ...
        if (stat.isDirectory() || stat.isFile()) {
          const foundRoot = current.normalize('NFC')
          logForDiagnosticsNoPII('info', 'find_git_root_completed', {
            duration_ms: Date.now() - startTime,
            stat_count: statCount,
            found: true,
          })
          // [fast-code] Cache all intermediate dirs so sibling lookups are O(1)
          let cacheDir = resolve(startPath)
          while (cacheDir !== foundRoot && cacheDir !== dirname(cacheDir)) {
            findGitRootImpl.cache.set(cacheDir, foundRoot)
...
// ---------------------------------------------------------------------------
// TTL-cached git status --porcelain
//
// getIsClean, getChangedFiles, and getFileStatus all need the same
// `git status --porcelain` output.  Instead of spawning the subprocess
// independently in each function (up to 3 redundant spawns on rapid
// successive calls), we cache the raw porcelain output for STATUS_TTL_MS
// and let callers derive what they need from the shared result.
// ---------------------------------------------------------------------------

const STATUS_TTL_MS = 5_000

let _cachedPorcelain: string | null = null
let _porcelainTimestamp = 0
let _porcelainPromise: Promise<string> | null = null

/**
 * Return raw `git status --porcelain` output, cached for STATUS_TTL_MS.
 * Concurrent callers within the TTL window share a single subprocess.
 */
async function getCachedPorcelain(): Promise<string> {
  const now = Date.now()
  if (_cachedPorcelain !== null && now - _porcelainTimestamp < STATUS_TTL_MS) {
    return _cachedPorcelain
  }
  // If a fetch is already in flight, piggyback on it
  if (_porcelainPromise) {
    return _porcelainPromise
  }
  _porcelainPromise = execFileNoThrow(
    gitExe(),
    ['--no-optional-locks', 'status', '--porcelain'],
    { preserveOutputOnError: false },
  ).then(({ stdout }) => {
    _cachedPorcelain = stdout
    _porcelainTimestamp = Date.now()
    _porcelainPromise = null
    return stdout
  })
  return _porcelainPromise
}

/** Invalidate the porcelain cache (e.g. after a write operation). */
export function invalidateGitStatusCache(): void {
  _cachedPorcelain = null
  _porcelainTimestamp = 0
  _porcelainPromise = null
}
...
export const getIsClean = async (options?: {
  ...
  if (options?.ignoreUntracked) {
    // Untracked-ignored variant can't use the shared cache (different flags).
  ...
  const stdout = await getCachedPorcelain()
  return stdout.trim().length === 0
}
...
export const stashToCleanState = async (message?: string): Promise<boolean> => {
  try {
    const stashMessage =
      message || `Fast Code auto-stash - ${new Date().toISOString()}`
    ...
    // Stash mutates the working tree — invalidate the cached porcelain output
    // so subsequent callers see the clean state.
