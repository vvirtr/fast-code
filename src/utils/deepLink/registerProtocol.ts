...
const APP_NAME = 'Fast Code URL Handler'
const DESKTOP_FILE_NAME = 'claude-code-url-handler.desktop'
const MACOS_APP_NAME = 'Fast Code URL Handler.app'
...
<dict>
  ...
  <array>
    <dict>
      ...
      <string>Fast Code Deep Link</string>
...
export async function ensureDeepLinkProtocolRegistered(): Promise<void> {
  ...
  // ~/.fast (per-machine, not synced) rather than ~/.fast.json (can sync).
