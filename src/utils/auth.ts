 ...
 * to the user's ~/.fast/settings.json API-key config (apiKeyHelper,
...
export function isAnthropicAuthEnabled(): boolean {
  ...
  // ~/.fast settings (apiKeyHelper, settings.env.ANTHROPIC_API_KEY) MUST NOT
 ...
 * from ~/.fast/settings.json or project settings is ignored.
 */
    ...
    refreshProc.on('close', (code, signal) => {
      if (code === 0) {
        ...
        const message = timedOut
          ...
          : chalk.red(
              'Error running awsAuthRefresh (in settings or ~/.fast.json):',
            )
  ...
  try {
    ...
    try {
      ...
      const message = chalk.red(
        'Error getting AWS credentials from awsCredentialExport (in settings or ~/.fast.json):',
      )
    ...
    refreshProc.on('close', (code, signal) => {
      if (code === 0) {
        ...
        const message = timedOut
          ...
          : chalk.red(
              'Error running gcpAuthRefresh (in settings or ~/.fast.json):',
            )
        // biome-ignore lint/suspicious/noConsole:: intentional console output
        console.error(message)
        authStatusManager.endAuthentication(false)
...
async function invalidateOAuthCacheIfDiskChanged(): Promise<void> {
  // [fast-code] Skip disk staleness check when token has ample validity.
  // On macOS (keychain path), the stat ENOENT clears the memoize on every
  // API request, causing falls through to the 30s keychain cache. When we
  // already hold a token valid for >10 min, there is no reason to check disk.
  const cached = getClaudeAIOAuthTokens.cache?.get?.(undefined)
  if (
    cached &&
    cached.expiresAt &&
    cached.expiresAt - Date.now() > 10 * 60 * 1000
  ) {
    return // Token valid for >10 min, no need to check disk
  }
...
export async function validateForceLoginOrg(): Promise<OrgValidationResult> {
  ...
  // in ~/.fast.json is user-writable and cannot be trusted.
