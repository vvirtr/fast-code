 ...
 * env vars that the remote's ~/.fast settings.env MUST NOT clobber (see
 ...
 * user's ~/.fast/settings.json can't redirect requests away from the
 ...
 *
 * - userSettings (~/.fast/settings.json): controlled by the user, not project-specific
...
export function applySafeConfigEnvironmentVariables(): void {
  ...
  // Global config (~/.fast.json) is user-controlled. In CCD mode,
  ...
  // doesn't get clobbered by ~/.fast/settings.json env (gh#217). policy/flag
