...
function ripGrepRaw(
  ...
  if (argv0) {
    ...
    child.on('close', (code, signal) => {
      ...
      const stdout = Buffer.concat(stdoutChunks).toString('utf8', 0, Math.min(stdoutLen, MAX_BUFFER_SIZE))
      const stderr = Buffer.concat(stderrChunks).toString('utf8', 0, Math.min(stderrLen, MAX_BUFFER_SIZE))
      if (code === 0 || code === 1) {
...
/**
 * Stream-based ripgrep that collects up to `maxLines` result lines, then kills
 * the rg process. For queries where the caller only needs the first N results
 * (e.g. head_limit=250 on a 100K-match pattern), this avoids buffering a 20MB
 * stdout string only to discard 99.7% of it.
 *
 * Falls back to the buffered `ripGrep()` when maxLines is not set (0/undefined)
 * or when offset > 0 (streaming can't skip efficiently).
 */
export async function ripGrepLimited(
  args: string[],
  target: string,
  abortSignal: AbortSignal,
  maxLines: number,
  ...
  // Create a child AbortController so we can kill rg when we have enough lines
  // while still respecting the parent signal.
  const childController = new AbortController()
  const onParentAbort = () => childController.abort()
  abortSignal.addEventListener('abort', onParentAbort, { once: true })

  try {
    await ripGrepStream(args, target, childController.signal, lines => {
    ...
    // AbortError from our own early-exit is expected, not an error
    if ((e as Error)?.name === 'AbortError' && collected.length >= maxLines) {
      return collected
    }
    // AbortError from parent signal — rethrow so caller handles it
