...
function cleanupTerminalModes(): void {
  ...
  try {
    ...
    // [fast-code] Always force exit alt screen + clear — catches race
    // conditions where isAltScreenActive is stale or unmount didn't
    // fully clean up. The clear (\x1b[2J\x1b[H) wipes any UI remnants.
    writeSync(1, EXIT_ALT_SCREEN + '\x1b[2J\x1b[H')
