...
export function getClaudeSkillScope(
  ...
  const bases = [
    ...
    {
      dir: expandPath(join(homedir(), '.fast', 'skills')),
      prefix: '~/.fast/skills/',
    },
  ]
 ...
 * Path format: ~/.fast/projects/{sanitized-cwd}/...
 */
...
export function checkWritePermissionForTool<Input extends AnyObject>(
  ...
  if (claudeFolderAllowRule) {
    ...
    // Accepts both the broad patterns ('/.claude/**', '~/.fast/**') and
...
export function checkEditableInternalPath(
  ...
  // var value must itself resolve under ~/.fast/jobs/. Symlink guard: every
  ...
  if (feature('TEMPLATES')) {
    ...
    if (jobDir) {
      ...
      // the case where ~/.fast is a symlink (e.g. to /data/claude-config).
  ...
  // ~/.fast/, which is in DANGEROUS_DIRECTORIES. The CLAUDE_COWORK_MEMORY_PATH_OVERRIDE
  ...
  // .claude/ only (not ~/.fast/) since launch.json is per-project.
  if (
...
export function checkReadableInternalPath(
  ...
  // Tasks directory (~/.fast/tasks/) for swarm task coordination
  ...
  // Teams directory (~/.fast/teams/) for swarm coordination
