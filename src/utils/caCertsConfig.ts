 ...
 * user-controlled files (~/.fast/settings.json and ~/.fast.json),
 ...
 *
 * We read from global config (~/.fast.json) and user settings
 * (~/.fast/settings.json). These are user-controlled files that don't
...
function getExtraCertsPathFromConfig(): string | undefined {
  try {
    ...
    // Only read from user-controlled settings (~/.fast/settings.json),
