...
import { loadTranscriptAsync } from './sessionLoadAsync.js'
import {
...
export async function loadTranscriptFile(
  ...
  try {
    // Offload the heavy I/O + parse pipeline to a Worker thread when available
    // (Bun environment). This avoids blocking the main thread for 50-200ms on
    // large sessions during --resume/--continue. Falls back to main-thread
    // execution in non-Bun environments or if the Worker fails.
    const disablePrecompactSkip = isEnvTruthy(
      process.env.CLAUDE_CODE_DISABLE_PRECOMPACT_SKIP,
    )
    const {
      entries,
      metaEntries,
      hasPreservedSegment: _hasPreservedSegment,
    } = await loadTranscriptAsync<Entry>(
      filePath,
