...
export async function getImageFromClipboard(): Promise<ImageWithDimensions | null> {
  ...
  try {
    ...
    // Detect format from magic bytes (use buffer directly to avoid redundant base64 decode)
    const mediaType = detectImageFormatFromBuffer(resized.buffer)
...
export async function tryReadImageFromPath(
  ...
  // Detect format from the actual file contents using magic bytes (use buffer directly to avoid redundant base64 decode)
  const mediaType = detectImageFormatFromBuffer(resized.buffer)
  return {
    path: imagePath,
    base64: base64Image,
