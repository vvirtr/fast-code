...
// [fast-code] Cache resized image results by content hash + target params
const resizeCache = new Map<string, { buffer: Buffer, mediaType: string }>()
const RESIZE_CACHE_MAX = 20
...
export async function maybeResizeAndDownsampleImageBuffer(
  ...
  // Quick hash of first 1KB + length for cache key
  const cacheKey = hashContent(imageBuffer.subarray(0, Math.min(1024, imageBuffer.length)).toString('base64') + imageBuffer.length)
  ...
  try {
    ...
    if (!metadata.width || !metadata.height) {
      if (originalSize > IMAGE_TARGET_RAW_SIZE) {
        ...
        const compressedBuffer = await sharp(imageBuffer)
          .jpeg({ quality: 80 })
          .toBuffer()
        const result = { buffer: compressedBuffer, mediaType: 'jpeg' }
      ...
      const result = { buffer: imageBuffer, mediaType: normalizedMediaType }
      resizeCache.set(cacheKey, result)
      if (resizeCache.size > RESIZE_CACHE_MAX) {
        resizeCache.delete(resizeCache.keys().next().value!)
      }
    ...
    if (base64Size <= API_IMAGE_MAX_BASE64_SIZE && !overDim) {
      ...
      const result = { buffer: imageBuffer, mediaType: normalizedExt }
      resizeCache.set(cacheKey, result)
      if (resizeCache.size > RESIZE_CACHE_MAX) {
        resizeCache.delete(resizeCache.keys().next().value!)
      }
