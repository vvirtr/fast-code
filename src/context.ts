...
export const getGitStatus = memoize(async (): Promise<string | null> => {
  ...
  try {
    ...
    // Previously 3 subprocess spawns: status, log, config user.name.
    // - user.name removed: only used for the optional "Git user:" display
    //   line in the system prompt, not worth a subprocess spawn.
    // - branch & defaultBranch are already filesystem-based (zero spawns).
    // - status + log remain as 2 parallel spawns (combining into 1 via
    //   shell would break Windows and add sh startup overhead).
