...
let cachedFuse: { sources: unknown; fuse: Fuse<any> } | null = null
...
export async function generateUnifiedSuggestions(
  ...
  if (nonFileSources.length > 0) {
    ...
    if (!cachedFuse || cachedFuse.sources !== nonFileSources) {
      cachedFuse = {
        sources: nonFileSources,
        fuse: new Fuse(nonFileSources, fuseOptions),
      }
    }
    const fuse = cachedFuse.fuse
