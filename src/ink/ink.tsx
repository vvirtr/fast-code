...
const noop = (): void => {};
import { throttle } from '../utils/lodashNative.js';
...
export default class Ink {
  ...
  private lastYogaWidth = -1; // Track last width set on yoga root for dirty-skip optimization
  ...
  constructor(private readonly options: Options) {
    ...
    this.rootNode.onComputeLayout = () => {
      ...
      if (this.rootNode.yogaNode) {
        // [fast-code] Skip layout when no yoga node is dirty and width unchanged
        // (e.g., spinner animation frames where only text content changes but
        // layout dimensions are unchanged). Text content updates that need
        // re-measurement will have called markDirty() on their yoga node, which
        // propagates up to the root, so this check is safe.
        const widthChanged = this.terminalColumns !== this.lastYogaWidth;
        if (!widthChanged && !this.rootNode.yogaNode.isDirty()) {
          return; // No layout changes, skip calculateLayout entirely
        }
        const t0 = performance.now();
        this.rootNode.yogaNode.setWidth(this.terminalColumns);
        this.lastYogaWidth = this.terminalColumns;
