...
// [fast-code] Cache at module scope — terminal type never changes mid-session
const _isXtermJsHost = process.env.TERM_PROGRAM === 'vscode' || isXtermJs()
function isXtermJsHost(): boolean {
  return _isXtermJsHost
}

// [fast-code] Per-line wrapText cache. Key: line text. Value: wrapped result.
// Eliminates redundant wrapping of unchanged lines during streaming.
const wrapLineCache = new Map<string, string>()
const WRAP_LINE_CACHE_MAX = 4096
...
function wrapWithSoftWrap(
  ...
  for (const orig of origLines) {
    const cacheKey = `${orig}\0${maxWidth}\0${textWrap}`
    let wrapped = wrapLineCache.get(cacheKey)
    if (wrapped === undefined) {
      wrapped = wrapText(orig, maxWidth, textWrap)
      wrapLineCache.set(cacheKey, wrapped)
      if (wrapLineCache.size > WRAP_LINE_CACHE_MAX) {
        // Evict oldest entry
        wrapLineCache.delete(wrapLineCache.keys().next().value!)
      }
    }
