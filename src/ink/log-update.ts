...
export class LogUpdate {
  private state: State
  /**
   * Persisted across frames so that when only the character changes but
   * the style stays the same (e.g. spinner rotation), no SGR set+reset
   * sequences are emitted. Reset to `stylePool.none` when
   * `prevFrameContaminated` is true (stderr or other external writes may
   * have changed the terminal's style state).
   */
  private currentStyleId: number

  constructor(private readonly options: Options) {
    this.currentStyleId = options.stylePool.none
    this.state = {
      previousOutput: '',
    }
  }
  ...
  reset(): void {
    ...
    this.currentStyleId = this.options.stylePool.none
  }
  ...
  render(
    ...
    if (
      ...
      this.currentStyleId = stylePool.none
    ...
    // Reset persisted style when the previous frame's terminal state is
    // untrustworthy (stderr writes, selection overlay, forceRedraw, etc.).
    if (prevFrameContaminated) {
      this.currentStyleId = stylePool.none
    }
    let currentStyleId = this.currentStyleId
    let currentHyperlink: Hyperlink = undefined
    ...
    // Persist style state for next frame so that unchanged styles
    // (e.g. spinner color) don't emit redundant SGR set+reset.
